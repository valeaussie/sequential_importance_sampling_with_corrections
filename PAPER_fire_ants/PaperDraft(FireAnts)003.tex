...
For simplicity, we will call $(\vec{a}^{T_j}, \vec{z}^{T_j}) = \vec{y}^{T_j}$ and $(\vec{(x')}^{T_j}, \vec{(z')}^{T_j}) = \vec{(y')}^{T_j}$.

We define a probability $p^*$ on $\Omega^* = \Omega \times \Omega'$ (need to define these spaces) such that the marginal distribution of $p^*$ on $\Omega'$ is $p$, that is:

\begin{equation*}
    p(\vec{(y')}^{T_j} | \vec{Z}^{T_j}) = \int_{\pi^{-1}(\vec{y'})} p^*(\vec{(y')}^{T_j}, \vec{y}^{T_j} | \vec{Z}^{T_j})\; d \vec{y}^{T_j}.
\end{equation*}
