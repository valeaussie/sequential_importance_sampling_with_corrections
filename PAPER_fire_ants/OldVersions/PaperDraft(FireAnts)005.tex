...
The spread of invasive species to new ares disrupt the equilibrium of the native ecosystem and can have adverse effects on the economy and the environment of the affected region. The real spread of the invasion is often unknown making effective eradication programs difficult to implement. The history of the invasion is usually also only partially known, so determining the real effects of eradication efforts is usually hard. 

Rebuilding the history of an invasion accurately is useful in many other contexts, but in this paper we are focusing on a particular invasive specie that is one of the world's 100 worst invaders: the Red Imported Fire Ants (RIFA). 
...
\end{equation*}
where we must have $f_p\leq t_d$. Notice that this process is Markovian.
...
\end{equation}
The likelihood at time $T$, will then be that of an inhomogeneous Poisson process for the founding process with intensity $\lambda(x, y, t)$ (See Reinhart \cite{Reinhart} for a derivation of the likelihood for self-exciting spatial-temporal point processes).
...
\end{equation*}
We will now use the model introduced in chapter \ref{section:model} for the RIFA invasion that defines the transition of the system from one state to the next: the transitional density distribution $f(\vec{a}^{T_i} | \vec{A}^{T_{i-1}})$ will be the likelihood defined in equation (\ref{eq:like}). Notice that the model is Markovian, therefore each $\vec{a}^{T_i}$ is conditionally independent of $\vec{A}^{T_{i-2}} = (\vec{a}^{T_1}, \dots, \vec{a}^{T_{i-2}})$, given $\vec{a}^{T_{i-1}}$ and we will therefore have $f(\vec{a}^{T_i} | \vec{A}^{T_{i-1}}) = f(\vec{a}^{T_i} | \vec{a}^{T_{i-1}})$. 

It follows that at time $T_1$ the prior distribution will be
